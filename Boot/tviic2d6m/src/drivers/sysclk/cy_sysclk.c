...
cy_en_syspm_status_t Cy_SysClk_DeepSleepCallback(cy_stc_syspm_callback_params_t *callbackParams)
{
        ...
        { /* Indicating that we can go into DeepSleep. Before doing so ... */
            ...
            for (fllpll = 0ul; fllpll < (SRSS_NUM_PLL + 1ul); fllpll++)
            {
                ...
                if (0ul != ((fllpll == 0ul) ? (SRSS->unCLK_FLL_CONFIG.stcField.u1FLL_ENABLE): 
                                              (((fllpll > 0ul) && (fllpll <= SRSS_NUM_PLL400M)) ? (SRSS->CLK_PLL400M[fllpll-1u].unCONFIG.stcField.u1ENABLE): 
                                                                                                  (SRSS->unCLK_PLL_CONFIG[fllpll - (SRSS_NUM_PLL400M + 1ul)].stcField.u1ENABLE))))
                {
    ...
    else if (callbackParams->mode == CY_SYSPM_AFTER_EXIT)
    {
        ...
        if (changedSourcePaths != 0u)
        {
            ...
            {
                /* for FLL and each PLL, */
                for (uint8_t fllpll = 0ul; fllpll < (SRSS_NUM_PLL + 1ul); fllpll++)
                {
                    /* If FLL or PLL is enabled, */
                    if (0ul != ((fllpll == 0ul) ? (SRSS->unCLK_FLL_CONFIG.stcField.u1FLL_ENABLE): 
                                                  (((fllpll > 0ul) && (fllpll <= SRSS_NUM_PLL400M)) ? (SRSS->CLK_PLL400M[fllpll-1u].unCONFIG.stcField.u1ENABLE): 
                                                                                                      (SRSS->unCLK_PLL_CONFIG[fllpll - (SRSS_NUM_PLL400M + 1ul)].stcField.u1ENABLE))))
                    {
                        ...
                        for (checkTimeout = TIMEOUTK; checkTimeout != 0ul; checkTimeout--)
                        {
                            if (true == ((fllpll == 0ul) ? Cy_SysClk_FllGetLockStatus() : (((fllpll > 0ul) && (fllpll <= SRSS_NUM_PLL400M)) ? Cy_SysClk_Pll400MGetLockStatus(fllpll): Cy_SysClk_PllGetLockStatus(fllpll))))
                            {
                                break;
                            }
                        }
