...
#define TIMING_MONZA_PMIC_ENABLE            (8)  // 0: Enable fastest timing, 8:Enable latest timing
...
void SystemInit (void)
{
    ...
    for(int8_t i_clkHfNo = 0ul; i_clkHfNo < SRSS_NUM_HFROOT; i_clkHfNo++)
    {
        if(i_clkHfNo == TIMING_MONZA_PMIC_ENABLE)
        {
            ...
            /***   Enabling MONZA PMIC     ***/
            #if defined(CY_327BGA_EVKLITE_rev_a)
            Cy_Power_SwitchToPmic(CY_SYSPMIC_VADJ_1V150, CY_POWER_PMIC_ENABLE_HIGH, CY_POWER_PMIC_STATUS_ABNORMAL_LOW, 0x200);
            #else
            Cy_Power_SwitchToPmic(CY_SYSPMIC_VADJ_1V150, CY_POWER_PMIC_ENABLE_HIGH, CY_POWER_PMIC_STATUS_ABNORMAL_LOW, 0);
            #endif
