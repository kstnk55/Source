...
*****************************************************************************/
static cy_en_srom_driver_status_t SwitchToExternal(cy_en_power_vadj_t                 voltageAdjust, 
                                                   ...
                                                   cy_en_power_mode_t                 mode)
{
    ...
    if(IsConfigured() == false) 
    {
        ...
        uint32_t OFFSET = 0U;
        /* CasePlayer2-E-SYN  : Declarations must appear at the beginning of the function or block. */
        ...
        OFFSET = CY_GET_REG32(0x17000730);
        if(OFFSET == 0)
        {
            #if   defined(CY_POWER_USES_REGHC_IP)
        ...
        retVal = Cy_Srom_CallApi_2(&configRegArgs, &configRegArgs2, NULL);

        /* 7. Check status code and wait for API Completion */
        if (CY_SROM_DR_SUCCEEDED != retVal)
        {
            /* Do nothing. Function ends here and returns current error status */
        }
        else
        {
            un_srom_api_args_t switchRegArgs = {0ul};
            /* (if waittime > 0x1FF)Set REGHC_PMIC_STATUS_WAIT */
            /*    Set wait time until the PMIC output settle. */
            ...
            retVal = Cy_Srom_CallApi(&switchRegArgs, NULL);

            /* 9. Check status code and wait for API Completion */
            /* Function ends here and returns current status */
        }
    }
    else
    {
 ...
 ** none
 *****************************************************************************/
cy_en_srom_driver_status_t Cy_Power_SwitchToTransistor(cy_en_power_vadj_t  voltageAdjust)
{

    return SwitchToExternal( voltageAdjust, 
