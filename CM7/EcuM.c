...
#if(ECUM_NUMBER_OF_CORES > 1u)
# define ECUM_GET_COREID()                                    GetCoreID()
#else
# define ECUM_GET_COREID()                                    OS_CORE_ID_0
#endif
...
/* Selectors for either the current or the last shutdown target and mode */
ECUM_LOCAL CONST(uint8, ECUM_CONST)       ECUM_CURR_SD_TARGET_AND_MODE = 0;
ECUM_LOCAL CONST(uint8, ECUM_CONST)       ECUM_LAST_SD_TARGET_AND_MODE = 1;
/* Displacement constant: from 32 bits use 8 bits for the shutdown target, remaining 24 bits for the mode */
ECUM_LOCAL CONST(uint8, ECUM_CONST)       ECUM_SEGMENT_SHIFT_SD_TARGET_AND_MODE = 24;
/* Holds the currently selected shutdown target and mode for shutdown operation */
ECUM_LOCAL VAR(uint32, ECUM_VAR_NO_INIT)  EcuM_CurrShutdownTargetAndMode;
/* Holds the shutdown target and mode set before the last reset */
ECUM_LOCAL VAR(uint32, ECUM_VAR_NO_INIT)  EcuM_LastShutdownTargetAndMode;
/* Marker whether a code sequence must be protected or not */
ECUM_LOCAL CONST(boolean, ECUM_CONST)     ECUM_PROTECTED_SEQUENCE = TRUE;
ECUM_LOCAL CONST(boolean, ECUM_CONST)     ECUM_UNPROTECTED_SEQUENCE = FALSE;
...
**********************************************************************************************************************/
/*! \brief       Return either the current or the last shutdown target and mode, depending on parameter currOrLastSelector
 *  \details     Selected shutdown target and mode were packed together either in variable
 *               EcuM_CurrShutdownTargetAndMode or in EcuM_LastShutdownTargetAndMode.
 *               The access on these variables can be protected using parameter ProtectSequence.
 *  \param[in]   protectSequence Selector for unproteced or protected access via ECUM_EXCLUSIVE_AREA_3
 *  \param[in]   currOrLastSelector Selector for read operation on either
 *               EcuM_CurrShutdownTargetAndMode or EcuM_LastShutdownTargetAndMode
 *  \param[out]  target Selected shutdown target
 *  \param[out]  mode Selected shutdown mode
 *  \pre         -
 *  \context     TASK
 *  \reentrant   TRUE
**********************************************************************************************************************/
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalGetShutdownTargetAndMode(
CONST(boolean, ECUM_CONST) protectSequence,
CONST(uint8, ECUM_CONST) currOrLastSelector,
P2VAR(EcuM_StateType, AUTOMATIC, ECUM_APPL_DATA) target,
P2VAR(EcuM_ModeType, AUTOMATIC, ECUM_APPL_DATA) mode);
 
/***********************************************************************************************************************
*  EcuM_InternalSetShutdownTargetAndMode
**********************************************************************************************************************/
/*! \brief      Store either the current or the last shutdown target and mode, depending on parameter currOrLastSelector
 *  \details    Selected shutdown target and mode will be packed together either in variable
 *              EcuM_CurrShutdownTargetAndMode or in EcuM_LastShutdownTargetAndMode
 *              The access on these variables can be protected using parameter protectSequence.
 *  \param[in]  protectSequence Selector for unproteced or protected access via ECUM_EXCLUSIVE_AREA_3
 *  \param[in]  currOrLastSelector Selector for store operation in either
 *              EcuM_CurrShutdownTargetAndMode or EcuM_LastShutdownTargetAndMode
 ...
 *  \reentrant  TRUE
**********************************************************************************************************************/
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalSetShutdownTargetAndMode(
CONST(boolean, ECUM_CONST) protectSequence,
CONST(uint8, ECUM_CONST) currOrLastSelector,
CONST(EcuM_StateType, ECUM_CONST) target,
CONST(EcuM_ModeType, ECUM_CONST) mode);
 ...
 *  \param[in]  haltMode The reduced power mode to be entered during the halted phase
 *  \pre        -
 *  \context    TASK|ISR
 *  \reentrant  FALSE
 *********************************************************************************************************************/
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalEnterHaltMode(CONST(EcuM_ModeType, ECUM_CONST) haltMode);
 ...
 *  \param[in]  pollMode The reduced power mode to be entered during the polling phase
 *  \pre        -
 *  \context    TASK|ISR
 *  \reentrant  FALSE
 *********************************************************************************************************************/
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalEnterPollingMode(CONST(EcuM_ModeType, ECUM_CONST) pollMode);
# endif
...
{
  /* ----- Local Variables ---------------------------------------------- */
  uint32 shutdownTargetAndMode = 0;
 
  /* ----- Implementation ----------------------------------------------- */
  /* Precondition: The Os must (still) be available on this core to protect the sequence. */
  if (protectSequence == TRUE)
  {
    EcuM_EnterCriticalArea_3();
  }
 
  if (currOrLastSelector == ECUM_CURR_SD_TARGET_AND_MODE)
  {
    /* Current shutdown target and mode is requested */
    shutdownTargetAndMode = EcuM_CurrShutdownTargetAndMode;
  }
  else
  {
    /* Last shutdown target and mode is requested */
    shutdownTargetAndMode = EcuM_LastShutdownTargetAndMode;
  }
  /* #10 Readout the shutdown target and mode from the corresponding variable. */
  *target = (uint8)(shutdownTargetAndMode >> ECUM_SEGMENT_SHIFT_SD_TARGET_AND_MODE); /* SBSW_ECUM_POINTERACCESS_SHUTDOWNTARGET */
  *mode = (uint8)(shutdownTargetAndMode); /* SBSW_ECUM_POINTERACCESS_SHUTDOWNTARGET */
...
{
  /* ----- Implementation ----------------------------------------------- */
  /* Precondition: The Os must (already and still) be available to protect the sequence. */
  if (protectSequence == TRUE)
  {
    EcuM_EnterCriticalArea_3();
  }
 
  /* #10 Store the parameters target and mode in the corresponding variable. */
  if (currOrLastSelector == ECUM_CURR_SD_TARGET_AND_MODE)
  {
    /* Current shutdown target and mode must be stored */
    EcuM_CurrShutdownTargetAndMode = ((uint32) target << ECUM_SEGMENT_SHIFT_SD_TARGET_AND_MODE) | ((uint32) mode);
  }
  else
  {
    /* Last shutdown target and mode must be stored */
    EcuM_LastShutdownTargetAndMode = ((uint32) target << ECUM_SEGMENT_SHIFT_SD_TARGET_AND_MODE) | ((uint32) mode);
  }
...
ECUM_LOCAL FUNC(Std_ReturnType, ECUM_CODE) EcuM_EnterSleep(VAR(uint8, AUTOMATIC) SuspendMode)
{
  ...
  EcuM_StateType shutdownTarget = 0;
  EcuM_ModeType shutdownMode = 0;
 
  EcuM_InternalGetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, &shutdownTarget, &shutdownMode); /* SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES */
...
# if (ECUM_ISDEF_WAKEUPSOURCEOFSLEEPMODELIST == STD_OFF)
    ...
    if ((EcuM_EnabledWakeups & (~((uint32)EcuM_GetWakeupSourceOfSleepModeList(shutdownMode)))) != 0u)
    {
      EcuM_DisableWakeupSources(EcuM_EnabledWakeups & (~((uint32)EcuM_GetWakeupSourceOfSleepModeList(shutdownMode))));
    }
# endif
    ...
    EcuM_EnableWakeupSources(EcuM_GetWakeupSourceOfSleepModeList(shutdownMode) & (~EcuM_EnabledWakeups));
    EcuM_EnabledWakeups = EcuM_GetWakeupSourceOfSleepModeList(shutdownMode);
...
# endif
      ...
      /* Store the shutdownTarget and shutdownMode in EcuM_LastShutdownTargetAndMode. */
      EcuM_InternalSetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_LAST_SD_TARGET_AND_MODE, shutdownTarget, shutdownMode);
 
# if(ECUM_POLLINGOFSLEEPMODELIST == STD_ON)
      /* Check if EcuM_EnterSleep is called by EcuM_GoPoll or EcuM_GoHalt. */
      if (SuspendMode == ECUM_ENTER_POLL)
      {
        EcuM_InternalEnterPollingMode(shutdownMode);
      }
      else
# endif
      {
        EcuM_InternalEnterHaltMode(shutdownMode);
      }
    }
    else
    {
...
ECUM_LOCAL_INLINE FUNC(Std_ReturnType, ECUM_CODE) EcuM_EnterSleepOnSlave(CoreIdType CoreID)
{
  EcuM_StateType shutdownTarget = 0;
  EcuM_ModeType shutdownMode = 0;
  Mcu_ModeType mcuModeOfSleepModeList = 0;
...
#  endif
    ...
    EcuM_InternalGetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, &shutdownTarget, &shutdownMode); /* SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES */
    mcuModeOfSleepModeList = EcuM_GetMcuModeOfSleepModeList(shutdownMode);
 
#if RESETMANAGER_OUTPUT_TASK_LOG
    /* Pnt01 */
    wvdRRAM_Task_Analyz_BSW_UpDateData(RRAM_TASKANALYSYS_BSW_POINT_1);
#endif
 
     /* FETA_EcuM_001 */
    do
    {
      EcuM_McuSetMode(mcuModeOfSleepModeList);
...
#if RESETMANAGER_OUTPUT_TASK_LOG
    /* Pnt02 */
    wvdRRAM_Task_Analyz_BSW_UpDateData(RRAM_TASKANALYSYS_BSW_POINT_2);
#endif
...
#if RESETMANAGER_OUTPUT_TASK_LOG
    /* Pnt03 */
    wvdRRAM_Task_Analyz_BSW_UpDateData(RRAM_TASKANALYSYS_BSW_POINT_3);
#endif
...
#if RESETMANAGER_OUTPUT_TASK_LOG
    /* Pnt04 */
    wvdRRAM_Task_Analyz_BSW_UpDateData(RRAM_TASKANALYSYS_BSW_POINT_4);
#endif
...
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalEnterHaltMode(CONST(EcuM_ModeType, ECUM_CONST) haltMode)
{
  Mcu_ModeType mcuModeOfSleepModeList = EcuM_GetMcuModeOfSleepModeList(haltMode);
  ...
  ECUM_DUMMY_STATEMENT_CONST(haltMode); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
}
...
ECUM_LOCAL_INLINE FUNC(void, ECUM_CODE) EcuM_InternalEnterPollingMode(CONST(EcuM_ModeType, ECUM_CONST) pollMode)
{
  ...
  EcuM_McuSetMode(EcuM_GetMcuModeOfSleepModeList(pollMode));
  ...
  while (ECUM_COMBINEDWAKEUPS == ECUM_WKSOURCE_NONE)
  {
    ...
    for (loopCount = 0u; loopCount < EcuM_GetSizeOfWakeupSourceList(); loopCount++)
    {
      if ((((EcuM_WakeupSourceType)1u << (EcuM_WakeupSourceType)loopCount) &
           (EcuM_GetWakeupSourceOfSleepModeList(pollMode))) != 0u)
      {
...
#  if (ECUM_POLLINGOFSLEEPMODELIST == STD_ON)
    if(EcuM_IsPollingOfSleepModeList(shutdownMode))
    {
...
#if(ECUM_NUMBER_OF_CORES > 1u)
  CoreIdType coreID = ECUM_GET_COREID();
...
#if(ECUM_NUMBER_OF_CORES > 1u)
    } /* (ECUM_GET_COREID() >= ECUM_CORE_ID_BSW) */
    else
    {
# if(ECUM_FIXED_BEHAVIOR == STD_OFF)
...
# endif
       ...
       * The current state is not a legal state for the API EcuM_SelectShutdownTarget.
       * To avoid errors at the uninit check, the global variables must be set directly at this place. */
      EcuM_InternalSetShutdownTargetAndMode(ECUM_UNPROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, EcuM_GetDefaultShutdownTarget(), EcuM_GetDefaultShutdownMode());
...
FUNC(void, ECUM_CODE) EcuM_Shutdown(void)
{
...
#endif
  {
    ...
    /* #22 Retrieve the value of the current shutdown target and mode and save these values for usage after a restart. */
    EcuM_InternalGetShutdownTargetAndMode(ECUM_UNPROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, &shutdownTarget, &shutdownMode); /* SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES */
    EcuM_InternalSetShutdownTargetAndMode(ECUM_UNPROTECTED_SEQUENCE, ECUM_LAST_SD_TARGET_AND_MODE, shutdownTarget, shutdownMode);
...
# endif
  ...
  else
  {
    ...
    if (targetState == ECUM_STATE_OFF)
    {
      ...
      EcuM_InternalSetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, targetState, 0u);
    }
    else
    {
      /* #30 Set the state of the current shutdown target. */
      /* #32 In case of ECUM_STATE_SLEEP / ECUM_STATE_RESET set the current shutdown mode to parameters value. */
      EcuM_InternalSetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, targetState, resetSleepMode);
    }
...
P2VAR(EcuM_ModeType, AUTOMATIC, ECUM_APPL_DATA) resetSleepMode)
{
  ...
  else
  {
    ...
    /* ----- Implementation ----------------------------------------------- */
    EcuM_InternalGetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, target, resetSleepMode); /* SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES */
    retval = E_OK;
  }
...
P2VAR(EcuM_ModeType, AUTOMATIC, ECUM_APPL_DATA) resetSleepMode)
{
  ...
  else
  {
    ...
    EcuM_InternalGetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_LAST_SD_TARGET_AND_MODE, target, resetSleepMode); /* SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES */
    retval = E_OK;
  }
...
# endif
  ...
  if ((ECUM_GET_COREID() == ECUM_CORE_ID_BSW)
# if(ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON)
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
    && (partitionIdx < EcuM_GetSizeOfPartitionData())
    && (EcuM_IsEcuMPartitionOfPartitionData(partitionIdx))
...
# endif /* ECUM_GODOWNALLOWEDOFFLEXUSER == STD_ON) */
...
#endif /* (ECUM_FIXED_BEHAVIOR != STD_ON */
      {
        ...
        /* #33 Only if the current ShutdownTarget is OFF, the EcuM checks again for pending wakeups. */
        EcuM_InternalGetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, &shutdownTarget, &shutdownMode); /* SBSW_ECUM_POINTER_TO_LOCAL_VARIABLES */
 
        if (shutdownTarget == ECUM_STATE_OFF)
        {
          ...
          if (ECUM_COMBINEDWAKEUPS != ECUM_WKSOURCE_NONE)
          {
            ...
            EcuM_InternalSetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, ECUM_STATE_RESET, EcuMConf_EcuMResetMode_ECUM_RESET_WAKEUP);
#else
            /* EcuMConf_EcuMResetMode_ECUM_RESET_WAKEUP is not configured. Set EcuM_CurrentShutdownMode to a not used mode.*/
            EcuM_InternalSetShutdownTargetAndMode(ECUM_PROTECTED_SEQUENCE, ECUM_CURR_SD_TARGET_AND_MODE, ECUM_STATE_RESET, 0xFFu);
#endif
          }
...
\ID SBSW_ECUM_POINTERACCESS_SHUTDOWNTARGET
 \DESCRIPTION The function writes to the 'target' and to 'mode' pointer parameters.
